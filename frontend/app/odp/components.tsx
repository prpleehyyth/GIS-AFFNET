...
export function OdpModal({ editId, form, setForm, existingOdps, isLoading, onClose, onSubmit, onMapClick }: ModalProps) {
  const [infoOdp, setInfoOdp] = useState<Odp | null>(null);
  ...
  // Klik marker ODC → auto-fill koordinat form jika form kosong
  const handleMarkerClick = useCallback((odp: Odp) => {
    setInfoOdp(odp);
    // Kalau form tipe ODP dan koordinat belum diisi, tawarkan auto-fill
  }, []);
            ...
            <div className={styles.mapSide}>
              ...
              >
                ...
                {form.latitude && form.longitude && (
                  <AdvancedMarker position={{ lat: parseFloat(form.latitude), lng: parseFloat(form.longitude) }}>
                ...
                {existingOdps
                  ...
                  .map(odp => (
                    <AdvancedMarker
                      ...
                      onClick={() => handleMarkerClick(odp)}
                    >
                      <Pin
                        background={TYPE_CFG[odp.type].pinBg}
                        borderColor="#fff"
                        glyphColor="#fff"
                        scale={odp.type === 'ODC' ? 1 : 0.8}
                      />
                    </AdvancedMarker>
                  ))}

                {/* InfoWindow saat marker diklik */}
                {infoOdp && (
                  <InfoWindow
                    position={{ lat: parseFloat(infoOdp.latitude), lng: parseFloat(infoOdp.longitude) }}
                    onCloseClick={() => setInfoOdp(null)}
                  >
                    <div style={{ fontFamily: 'var(--font)', padding: 4, minWidth: 140 }}>
                      <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 4 }}>{infoOdp.name}</div>
                      <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 8 }}>
                        {infoOdp.type} · {parseFloat(infoOdp.latitude).toFixed(5)}, {parseFloat(infoOdp.longitude).toFixed(5)}
                      </div>
                      {/* Tombol gunakan koordinat ODC ini untuk ODP yang sedang dibuat */}
                      {form.type === 'ODP' && infoOdp.type === 'ODC' && (
                        <button
                          onClick={() => {
                            onMapClick(parseFloat(infoOdp.latitude), parseFloat(infoOdp.longitude));
                            setForm({ ...form, odc_id: infoOdp.id });
                            setInfoOdp(null);
                          }}
                          style={{
